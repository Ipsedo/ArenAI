...
Engine::Engine(Level *level, AAssetManager *mgr)
  ...
  world =
    new btDiscreteDynamicsWorld(dispatcher, broadPhase, constraintSolver, collisionConfiguration);
...
void Engine::update(float delta) {
  ...
  for (Base *b: level->getEntities()) {
    if (!b->isInWorld()) world->addRigidBody(b);
    b->update();
  }

  for (Shooter *s: level->getShooters()) level->addBases(s->fire());
    ...
    if (isDead) {
      if (b->needExplosion()) {
        ...
        for (int i = 0; i < nb_particules; i++) {
          ...
          for (int j = i - 1; j >= 0; j--) explosionGroup[j]->setIgnoreCollisionCheck(p, true);
        }
      }
      deleteBase(b);
    }
