...
public:
  ...
  void processTriangle(btVector3 *triangle, int partid,
    ...
    glm::vec3 p1 =
        glm::vec3(triangle[0].getX(), triangle[0].getY(), triangle[0].getZ());
    glm::vec3 p2 =
        glm::vec3(triangle[1].getX(), triangle[1].getY(), triangle[1].getZ());
    glm::vec3 p3 =
        glm::vec3(triangle[2].getX(), triangle[2].getY(), triangle[2].getZ());
...
makeMapCInfo(btHeightfieldTerrainShape *map, btVector3 pos, btVector3 scale) {
  ...
  return btRigidBody::btRigidBodyConstructionInfo(0.f, motionState, map,
                                                  intertie);
}
...
btHeightfieldTerrainShape *makeTerrainShape(float *normalizedHeightValues,
                                            int width, int length,
                                            btVector3 scale) {
  auto map =
      new btHeightfieldTerrainShape(width, length, normalizedHeightValues, 1.f,
                                    0.f, 1.f, 1, PHY_FLOAT, false);
  map->setLocalScaling(btVector3(scale));
  return map;
}

Map::Map(float *normalizedHeightValues, int width, int length, btVector3 pos,
         btVector3 scale)
    : normalizedHeightValues(normalizedHeightValues),
      Ground(makeTerrainShape(normalizedHeightValues, width, length, scale),
             pos, scale) {}

glm::vec3 Map::getMinPos() { return minPos; }

glm::vec3 Map::getMaxPos() { return maxPos; }

Map::~Map() { delete[] normalizedHeightValues; }
