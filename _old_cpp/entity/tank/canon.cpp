...
Canon::Canon(AAssetManager *mgr, btDynamicsWorld *world, Base *turret,
             btVector3 turretPos)
    : Poly(
          [mgr](glm::vec3 scale) {
            string canonObjTxt = getFileText(mgr, "obj/cylinderZ.obj");
            return new btCylinderShapeZ(
                btVector3(canonScale.x, canonScale.y, canonScale.z));
          },
      ...
      hasClickedShoot(false), missile(makeMissileModel(mgr)), maxFramesFire(25),
      fireCounter(0), turret(turret), isHit(false) {
  ...
  hinge =
      new btHingeConstraint(*pBodyA, *pBodyB, pivotA, pivotB, axis, axis, true);
...
void Canon::update() {
  ...
  fireCounter =
      fireCounter + 1 <= maxFramesFire ? fireCounter + 1 : maxFramesFire;
...
Base *Canon::generateMissile() {
  ...
  Missile *m =
      new Missile(missile, glm::vec3(vec), missileScale, rotMat, 10.f, 1);
...
btRigidBody *Canon::getMissileCopy() { return generateMissile(); }
...
glm::mat4 Canon::getCamRot() {
  ...
  glm::mat3 rot =
      hinge->getHingeAngle() < LIMIT_ANGLE_CAMERA
          ? glm::mat3(turretModelMatrix * glm::rotate(glm::mat4(1.0f),
                                                      LIMIT_ANGLE_CAMERA,
