...
NormalMapModel *makeCanonModel(AAssetManager *mgr) {
  return new NormalMapModel(
    mgr, "obj/cylinderZ.obj", "textures/turret_tex.png", "textures/199_norm.png");
}
...
Canon::Canon(AAssetManager *mgr, btDynamicsWorld *world, Base *turret, btVector3 turretPos)
    : Poly(
      ...
      makeCanonModel(mgr), btVector3ToVec3(turretPos + canonRelPos), canonScale, glm::mat4(1.0f),
      canonMass, true),
      angle(0.f), added(0), respawn(false), pos(turretPos + canonRelPos), hasClickedShoot(false),
      missile(makeMissileModel(mgr)), maxFramesFire(25), fireCounter(0), turret(turret),
      isHit(false) {
...
vector<Base *> Canon::fire() {
  if (!hasClickedShoot || fireCounter < maxFramesFire) { return vector<Base *>(); }
  fireCounter = 0;
...
glm::mat4 Canon::getCamRot() {
  ...
  glm::mat3 rot = hinge->getHingeAngle() < LIMIT_ANGLE_CAMERA ? glm::mat3(
                    turretModelMatrix
