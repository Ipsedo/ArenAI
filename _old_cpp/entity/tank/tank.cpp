...
vector<Base *> Tank::getBases() {
  ...
  res.push_back(canon);
  for (Base *b: wheels) res.push_back(b);
  return res;
}
...
vector<Controls *> Tank::getControls() {
  ...
  res.push_back(turret);
  res.push_back(canon);
  for (Controls *c: wheels) res.push_back(c);
  return res;
}

Camera *Tank::getCamera() { return camera; }

vector<Shooter *> Tank::getShooters() { return vector<Shooter *>{canon}; }

vector<Drawable *> Tank::getHUDDrawables() { return vector<Drawable *>{curve}; }
