...
Wheel::Wheel(
    AAssetManager *mgr, btDynamicsWorld *world, Base *chassis, btVector3 chassisPos, btVector3 pos)
    : Poly(
        [mgr](glm::vec3 scale) {
        ...
        glm::vec3(wheelWidth, wheelRadius, wheelRadius), glm::mat4(1.0f), wheelMass, true),
      pos(pos), chassisPos(chassisPos), isMotorEnabled(true), isBraking(true), freeWheel(false),
      targetSpeed(0.f), hasReAccelerate(false), respawn(false) {
    setFriction(500.f);
