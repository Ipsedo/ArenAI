...
Wheel::Wheel(AAssetManager *mgr, btDynamicsWorld *world, Base *chassis,
    ...
    : Poly(
          ...
          glm::vec3(wheelWidth, wheelRadius, wheelRadius), glm::mat4(1.0f),
          wheelMass, true),
      pos(pos), chassisPos(chassisPos), isMotorEnabled(true), isBraking(true),
      freeWheel(false), targetSpeed(0.f), hasReAccelerate(false),
      respawn(false) {
  setFriction(500.f);
...
void Wheel::onInput(input in) {
  ...
  respawn = in.respawn;
  if (isMotorEnabled)
    hasReAccelerate = true;
  if (hasReAccelerate)
    isBraking = in.brake;
  if (isBraking)
    hasReAccelerate = false;
}
...
FrontWheel::FrontWheel(AAssetManager *mgr, btDynamicsWorld *world,
                       Base *chassis, btVector3 chassisPos, btVector3 pos)
...
ModelVBO *Wheel::makeWheelMesh(AAssetManager *mgr) {
  ...
  return new ModelVBO(cylObjText, wheelColor[0], wheelColor[1], wheelColor[2],
                      wheelColor[3]);
}

output Wheel::getOutput() { return {false}; }
