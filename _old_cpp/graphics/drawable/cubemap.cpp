...
void CubeMap::loadCubeMaptexture(AAssetManager *mgr, string cubaMapAssetPath) {
  ...
  GLenum idTxt[]{GL_TEXTURE_CUBE_MAP_POSITIVE_X, GL_TEXTURE_CUBE_MAP_POSITIVE_Y,
                 GL_TEXTURE_CUBE_MAP_POSITIVE_Z, GL_TEXTURE_CUBE_MAP_NEGATIVE_X,
                 GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z};
  string fileName[]{"posx.png", "posy.png", "posz.png", "negx.png", "negy.png", "negz.png"};
  for (int i = 0; i < 6; i++) {
    libpng_image tmp = readPNG(mgr, cubaMapAssetPath + fileName[i]);
    imgRGB img = toImgRGB(tmp);
    glTexImage2D(
      idTxt[i], 0, GL_RGB, img.width, img.height, 0, GL_RGB, GL_UNSIGNED_BYTE, img.pixels);
    delete[] tmp.data;
    delete[] tmp.rowPtrs;
    delete[] img.pixels;
  }
