...
void NormalMapModel::initTex(AAssetManager *mgr, string textureFile, string normalsFile) {
    ...
    glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RGB, img1.width, img1.height, 0, GL_RGB, GL_UNSIGNED_BYTE,
        img1.pixels);
    ...
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RGB, img2.width, img2.height, 0, GL_RGB, GL_UNSIGNED_BYTE,
        img2.pixels);
...
void NormalMapModel::initPrgm(AAssetManager *mgr) {
    ...
    GLuint vs =
        loadShader(GL_VERTEX_SHADER, getFileText(mgr, "shaders/normal_map_vs.glsl").c_str());
    GLuint fs =
