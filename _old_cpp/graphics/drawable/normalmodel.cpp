...
NormalMapModel::NormalMapModel(AAssetManager *mgr, string objFile,
                               string textureFile, string normalsFile)
...
void NormalMapModel::initTex(AAssetManager *mgr, string textureFile,
                             string normalsFile) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img1.width, img1.height, 0, GL_RGB,
               GL_UNSIGNED_BYTE, img1.pixels);
  ...
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, img2.width, img2.height, 0, GL_RGB,
               GL_UNSIGNED_BYTE, img2.pixels);
...
void NormalMapModel::initPrgm(AAssetManager *mgr) {
  ...
  GLuint vs = loadShader(
      GL_VERTEX_SHADER, getFileText(mgr, "shaders/normal_map_vs.glsl").c_str());
  GLuint fs =
...
void NormalMapModel::draw(gl_draw_info info) {
  ...
  glVertexAttribPointer(mNormalHandle, NORMAL_SIZE, GL_FLOAT, GL_FALSE, STRIDE,
  ...
  glUniformMatrix4fv(mMVMatrixHandle, 1, GL_FALSE,
                     glm::value_ptr(info.mv_matrix));

  glUniformMatrix4fv(mMVPMatrixHandle, 1, GL_FALSE,
                     glm::value_ptr(info.mvp_matrix));
...
void NormalMapModel::genBuffer(AAssetManager *mgr, string objFile) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, buffer);
  glBufferData(GL_ARRAY_BUFFER, packedData.size() * BYTES_PER_FLOAT,
               packedData.data(), GL_STATIC_DRAW);
