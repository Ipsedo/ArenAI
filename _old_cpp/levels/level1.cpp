...
Level1::Level1()
    : Level(), isInit(false), cibles(vector<Cible *>()), compass(vector<Compass *>()) {}
...
void Level1::init(bool isVR, AAssetManager *mgr, btDynamicsWorld *world) {
  ...
  for (int i = 0; i < img.allpixels.size(); i++) { array[i] = img.allpixels[i]; }
  delete[] tmp.rowPtrs;
  delete[] tmp.data;
  img.allpixels.clear();
  Map *sol =
    new Map(array, img.width, img.height, btVector3(0.f, 0.f, 0.f), btVector3(0.8f, 20.f, 0.8f));
...
vector<Controls *> Level1::getControls() {
  if (!isInit) return vector<Controls *>();
...
vector<Shooter *> Level1::getShooters() {
  if (!isInit) return vector<Shooter *>();
  return tank->getShooters();
}

vector<Base *> Level1::getEntities() {
  if (!isInit) return vector<Base *>();
  return entities;
}

vector<Drawable *> Level1::getDrawables() {
  if (!isInit) return vector<Drawable *>();

  vector<Drawable *> d;
  for (Base *b: entities) d.push_back(b);
  for (int i = 0; i < compass.size(); i++)
    if (!cibles[i]->isWon()) d.push_back(compass[i]);
  d.push_back(map);
  for (Drawable *dr: tank->getHUDDrawables()) d.push_back(dr);
  return d;
}
...
bool Level1::won() {
  bool won = true;
  for (Cible *c: cibles) won &= c->isWon();
  return won;
}

bool Level1::lose() { return false; }
...
glm::vec3 Level1::getLightPos() { return glm::vec3(0, 200, 0); }
