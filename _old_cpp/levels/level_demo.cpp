...
void LevelDemo::init(bool isVR, AAssetManager *mgr, btDynamicsWorld *world) {
  ...
  for (int i = 0; i < nbEntity; i++) {
    ...
    entities.push_back(new Box(mgr, glm::vec3(x, HEIGHT_SPAWN, z),
                               glm::vec3(scale), id, mass));
  }
  for (int i = 0; i < nbEntity; i++) {
    float x = spawnRange * (float)rand() / RAND_MAX - spawnRange * 0.5f;
    float z = spawnRange * (float)rand() / RAND_MAX - spawnRange * 0.5f;
    ...
    entities.push_back(new Cylinder(mgr, glm::vec3(x, HEIGHT_SPAWN, z),
                                    glm::vec3(scale), id, mass));
  }
  for (int i = 0; i < nbEntity; i++) {
    float x = spawnRange * (float)rand() / RAND_MAX - spawnRange * 0.5f;
    ...
    entities.push_back(new Cone(mgr, glm::vec3(x, HEIGHT_SPAWN, z),
                                glm::vec3(scale), id, mass));
  }
  for (int i = 0; i < nbEntity; i++) {
    float x = spawnRange * (float)rand() / RAND_MAX - spawnRange * 0.5f;
    ...
    float scale = 2.f * (float)rand() / RAND_MAX;
    float mass = maxMass * float(rand()) / RAND_MAX;
    entities.push_back(new Sphere(mgr, glm::vec3(x, HEIGHT_SPAWN, z),
                                  glm::vec3(scale), id, mass));
  }
...
bool LevelDemo::won() { return false; }

bool LevelDemo::lose() { return false; }
...
glm::vec3 LevelDemo::getLightPos() { return glm::vec3(500, 400, 200); }
