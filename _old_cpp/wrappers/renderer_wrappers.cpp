...
Java_com_samuelberrien_phyvr_MainWrappers_makeRenderer(JNIEnv *env,
                                                       jobject instance,
                                                       jlong levelPtr) {
...
Java_com_samuelberrien_phyvr_MainWrappers_willDrawRenderer(
    JNIEnv *env, jobject instance, jlong rendererPtr, jfloatArray mHeadView_,
    jboolean VR) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_samuelberrien_phyvr_MainWrappers_drawRenderer(
    JNIEnv *env, jobject instance, jlong rendererPtr,
    jfloatArray mEyeProjectionMatrix_, jfloatArray mEyeViewMatrix_,
    jfloatArray myLighPosInEyeSpace_, jfloatArray mCameraPos_) {
  jfloat *mEyeProjectionMatrix =
      env->GetFloatArrayElements(mEyeProjectionMatrix_, NULL);
  jfloat *mEyeViewMatrix = env->GetFloatArrayElements(mEyeViewMatrix_, NULL);
  jfloat *myLighPosInEyeSpace =
      env->GetFloatArrayElements(myLighPosInEyeSpace_, NULL);
  ...
  env->ReleaseFloatArrayElements(mEyeProjectionMatrix_, mEyeProjectionMatrix,
                                 0);
...
extern "C" JNIEXPORT void JNICALL
Java_com_samuelberrien_phyvr_MainWrappers_freeRenderer(JNIEnv *env,
                                                       jobject instance,
                                                       jlong rendererPtr) {
