...
CoreEngine::CoreEngine(AAssetManager *mgr)
    : is_paused(true),
      camera(std::make_shared<StaticCamera>(glm::vec3(0., 10., 0.),
                                            glm::vec3(0., 9., 1.),
                                            glm::vec3(0., 1., 0.))),
      renderer(std::nullptr_t()), physic_engine(), controller_engine(), items(),
      dev(), rng(dev()) {

  auto map = std::make_shared<HeightMapItem>(
      "height_map", mgr, "heightmap/heightmap6.png", glm::vec3(0., 40., 0.),
      glm::vec3(10., 200., 10.));

  auto sphere_shape = std::make_shared<ObjShape>(mgr, "obj/sphere.obj");
