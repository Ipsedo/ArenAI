...
Cylinder::Cylinder(const btRigidBody::btRigidBodyConstructionInfo &constructionInfo, btDefaultMotionState *motionState,
...
Cylinder::MakeCylinder(AAssetManager *mgr, glm::vec3 pos, glm::vec3 scale, glm::mat4 rotationMatrix, float mass) {
	...
	return new Cylinder(get<0>(cinfo), get<1>(cinfo), modelVBO, scale);
}
