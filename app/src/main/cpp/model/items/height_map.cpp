...
HeightMapItem::HeightMapItem(std::string name, AAssetManager *mgr,
  ...
  auto map = new btHeightfieldTerrainShape(img.width, img.height, img.pixels,
                                           1.f, 0.f, 1.f, 1, PHY_FLOAT, false);
  map->setLocalScaling(btVector3(scale.x, scale.y, scale.z));
  map->processAllTriangles(this, btVector3(-1000., -1000., -1000.),
  ...
  btRigidBody::btRigidBodyConstructionInfo info(0.f, motionState, map,
                                                intertie);
...
void HeightMapItem::processTriangle(btVector3 *triangle, int partid,
                                    int triangleindex) {
  glm::vec3 p1 =
