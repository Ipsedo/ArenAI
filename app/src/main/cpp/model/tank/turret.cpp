...
TurretItem::TurretItem(std::string name, AAssetManager *mgr, glm::vec3 pos,
                       glm::vec3 rel_pos, glm::vec3 scale, float mass,
                       btRigidBody *chassis)
    : ConvexItem(std::move(name),
                 std::make_shared<ObjShape>(mgr, "obj/anubis_turret.obj"), pos,
                 scale, mass),
      angle(0.f) {
  ...
  hinge = new btHingeConstraint(*chassis, *ConvexItem::get_body(),
                                chassis_pivot, turret_pivot, axis, axis, true);
  hinge->setLimit(0, 0);
}

void TurretItem::on_input(const user_input &input) {
  angle += -input.right_joystick.x * 1.5e-2f;
