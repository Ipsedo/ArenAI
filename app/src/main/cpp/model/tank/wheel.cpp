...
WheelItem::WheelItem(std::string name, AAssetManager *mgr, glm::vec3 pos,
                     glm::vec3 rel_pos, glm::vec3 scale, float mass,
                     btRigidBody *chassis)
    : ConvexItem(std::move(name),
                 std::make_shared<ObjShape>(mgr, "obj/anubis_wheel.obj"), pos,
                 scale, mass) {
  ...
  hinge = new btGeneric6DofSpring2Constraint(*chassis, *ConvexItem::get_body(),
                                             frame_in_chassis, frame_in_wheel,
                                             RO_XYZ);
...
DirectionalWheelItem::DirectionalWheelItem(std::string name, AAssetManager *mgr,
                                           glm::vec3 pos, glm::vec3 rel_pos,
                                           glm::vec3 scale, float mass,
                                           btRigidBody *chassis)
    : WheelItem(std::move(name), mgr, pos, rel_pos, scale, mass, chassis) {}
