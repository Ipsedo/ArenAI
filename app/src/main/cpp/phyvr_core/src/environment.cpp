...
BaseTanksEnvironment::BaseTanksEnvironment(
    ...
    : nb_tanks(nb_tanks), threads_running(true),
      thread_barrier{static_cast<std::ptrdiff_t>(nb_tanks + 1)}, tank_factories(), tank_renderers(),
      pool(), physic_engine(std::make_shared<PhysicEngine>(threads_num)), dev(), rng(dev()),
