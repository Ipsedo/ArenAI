...
TankFactory::TankFactory(
  ...
  for (auto &[wheel_name, is_directional, wheel_pos]: wheel_config) {
    ...
    wheel = make_wheel_(
      is_directional, tank_prefix_name + "_" + wheel_name, file_reader, wheel_pos + chassis_pos,
      wheel_pos, wheel_scale, wheel_mass, chassis_item->get_body());
  ...
  auto turret = std::make_shared<TurretItem>(
    tank_prefix_name, file_reader, chassis_pos + turret_pos, turret_pos, scale * turret_scale, 200,
    chassis_item->get_body());
