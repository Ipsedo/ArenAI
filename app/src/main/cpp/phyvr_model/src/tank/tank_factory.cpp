...
TankFactory::TankFactory(const std::shared_ptr<AbstractFileReader> &file_reader,
  ...
  // chassis
  auto chassis_item =
      std::make_shared<ChassisItem>(file_reader, chassis_pos, scale, 2000.f);
  ...
  for (auto &[wheel_name, is_directional, wheel_pos] : wheel_config) {
    ...
    wheel = make_wheel_(is_directional, wheel_name, file_reader,
  ...
  auto turret = std::make_shared<TurretItem>(
      "turret", file_reader, chassis_pos + turret_pos, turret_pos,
  ...
  auto canon = std::make_shared<CanonItem>(
      "canon", file_reader, chassis_pos + turret_pos + canon_pos, canon_pos,
