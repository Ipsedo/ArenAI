...
TankFactory::TankFactory(const std::shared_ptr<AbstractFileReader> &file_reader,
                         const std::string &tank_prefix_name,
  ...
  auto chassis_item = std::make_shared<ChassisItem>(
      tank_prefix_name, file_reader, chassis_pos, scale, 2000.f);
  ...
  for (auto &[wheel_name, is_directional, wheel_pos] : wheel_config) {
    ...
    wheel = make_wheel_(is_directional, tank_prefix_name + "_" + wheel_name,
                        file_reader, wheel_pos + chassis_pos, wheel_pos,
  ...
  auto turret = std::make_shared<TurretItem>(
      ...
      scale * turret_scale, 200, chassis_item->get_body());
  ...
  auto canon = std::make_shared<CanonItem>(
      tank_prefix_name, file_reader, chassis_pos + turret_pos + canon_pos,
      canon_pos, scale * canon_scale, 50, turret->get_body());
