...
WheelItem::WheelItem(const std::string &prefix_name,
    ...
    : ConvexItem(
          prefix_name + "_wheel",
          std::make_shared<ObjShape>(file_reader, "obj/anubis_wheel.obj"), pos,
          scale, mass) {
...
DirectionalWheelItem::DirectionalWheelItem(
    ...
    btRigidBody *chassis)
    : WheelItem(std::move(name), file_reader, pos, rel_pos, scale, mass,
                chassis) {}
