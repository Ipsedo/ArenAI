...
public:
  ButtonDrawable(const std::shared_ptr<AbstractFileReader> &file_reader,
                 std::function<button(void)> get_input, glm::vec2 center_px,
                 float size_px);
...
public:
  JoyStickDrawable(const std::shared_ptr<AbstractFileReader> &file_reader,
