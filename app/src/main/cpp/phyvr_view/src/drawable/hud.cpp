...
ButtonDrawable::ButtonDrawable(
    ...
    : get_input(std::move(get_input)), center_x(center_px.x),
      center_y(center_px.y), size(size_px), nb_points(128) {

  program = Program::Builder(file_reader, "shaders/simple_vs.glsl",
...
JoyStickDrawable::JoyStickDrawable(
    const std::shared_ptr<AbstractFileReader> &file_reader,
    std::function<joystick(void)> get_input_px, glm::vec2 center_px,
  ...
  program = Program::Builder(file_reader, "shaders/simple_vs.glsl",
                ...
                .add_uniform("u_mvp_matrix")
                .add_attribute("a_position")
                .add_buffer("square_buffer", {-1.f, 1.f, 0.f, 1.f, 1.f, 0.f,
