...
std::shared_ptr<Program> Program::Builder::build() {
  ...
  program->fragment_shader_id = load_shader(file_reader, GL_FRAGMENT_SHADER, fragment_shader_path);
  ...
  for (const auto &[name, files_path]: cube_textures) {
    ...
    for (const auto &file: files_path) {
      ...
      glTexImage2D(
        file_to_tex_id[file_name], 0, GL_RGBA, img.width, img.height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
        img.pixels);
...
void Program::attrib(
  const std::string &name, const std::string &buffer_name, int data_size, int stride, int offset) {
