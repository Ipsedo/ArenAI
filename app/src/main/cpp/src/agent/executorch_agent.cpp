...
float erfinv(float x) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    float deriv =
        (2.f / std::sqrt(static_cast<float>(M_PI))) * std::exp(-y * y);
    y -= err / deriv;
  }
  return y;
}
...
float phi(float z) {
  return std::exp(-0.5f * std::pow(z, 2.0f)) /
...
float theta_inv(float theta) {
  return std::sqrt(2.0f) * erfinv(2.0f * theta - 1.0f);
}

float truncated_normal_sample(std::random_device rng,
                              std::uniform_real_distribution<float> dist,
                              float mu, float sigma, float min_value,
                              float max_value) {

  const auto safe_sigma = std::clamp(sigma, SIGMA_MIN, SIGMA_MAX);

  const auto alpha = std::clamp((min_value - mu) / safe_sigma,
                                -ALPHA_BETA_BOUND, ALPHA_BETA_BOUND);
  const auto beta = std::clamp((max_value - mu) / safe_sigma, -ALPHA_BETA_BOUND,
                               ALPHA_BETA_BOUND);

  const auto cdf = std::clamp(
      theta(alpha) + dist(rng) * (theta(beta) - theta(alpha)), 0.f, 1.f);
