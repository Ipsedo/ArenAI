...
UserGameTanksEnvironment::UserGameTanksEnvironment(struct android_app *app)
    : BaseTanksEnvironment(
          ...
          std::make_shared<AndroidGLContext>(app->window), 4, 3),
...
int32_t UserGameTanksEnvironment::on_input(struct android_app *app,
                                           AInputEvent *event) {
...
void UserGameTanksEnvironment::on_reset_physics(
  ...
  tank_factory = std::make_unique<TankFactory>(file_reader, "player",
                                               glm::vec3(0., -40., 40));
...
void UserGameTanksEnvironment::on_reset_drawables(
  ...
  player_renderer = std::make_unique<PlayerRenderer>(
      gl_context, ANativeWindow_getWidth(app->window),
      ANativeWindow_getHeight(app->window), glm::vec3(200, 300, 200),
