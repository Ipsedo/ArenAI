...
HUDButton::HUDButton(
    ...
    : size(dp_to_px(config, size_dp)),
      ...
      center_y(dp_to_px(config, margin_dp) + size / 2.f), height(height_px), touched(false),
      pointer_id(-1), pressed(false) {}
...
bool HUDButton::is_inside_(float x, float y) const {
    return (center_x - size / 2.f < x) && (x < center_x + size / 2.f) && (center_y - size / 2.f < y)
           && (y < center_y + size / 2.f);
}
...
HUDButton::get_hud_drawable(const std::shared_ptr<AbstractFileReader> &file_reader) {
    return std::make_unique<ButtonDrawable>(
        file_reader, [this]() { return get_input(); }, glm::vec2(center_x, center_y), size);
}
