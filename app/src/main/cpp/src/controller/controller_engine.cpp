...
ControllerEngine::ControllerEngine(AConfiguration *config, int width, int height) : controllers() {
    drive_joystick = std::make_shared<HUDJoyStick>(config, width, height, 40, 150, 50);
...
void ControllerEngine::add_controller(const std::shared_ptr<Controller> &controller) {
...
ControllerEngine::get_hud_drawables(const std::shared_ptr<AbstractFileReader> &file_reader) {
