...
std::unique_ptr<HUDDrawable> HUDJoyStick::get_hud_drawable(
    const std::shared_ptr<AbstractFileReader> &file_reader) {
  return std::move(std::unique_ptr<HUDDrawable>(new JoyStickDrawable(
      file_reader, [this] { return get_input_px(); },
      {center_pos_x, center_pos_y}, size, stick_size)));
}
...
ScreenJoyStick::ScreenJoyStick(
    ...
    : touched(false), width(float(width)), height(float(height)),
      pointer_id(-1), last_x(0.f), last_y(0.f), x_value(0.f), y_value(0.f),
