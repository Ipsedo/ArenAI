...
CoreEngine::CoreEngine(struct android_app *app)
    : file_reader(
      ...
      tank_factory(file_reader, glm::vec3(0., -40., 40)), is_paused(true),
      camera(std::nullptr_t()), renderer(std::nullptr_t()),
...
void CoreEngine::_new_view(struct android_app *app) {
  renderer = std::make_unique<NormalRenderer>(app->window, camera);
  controller_engine = std::make_unique<ControllerEngine>(
      app->config, renderer->get_width(), renderer->get_height());
  ...
  for (auto &hud_drawable : controller_engine->get_hud_drawables(file_reader))
