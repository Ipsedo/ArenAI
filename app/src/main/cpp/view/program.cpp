...
Program::Builder::Builder(AAssetManager *mgr, std::string vertex_shader_path,
    ...
    : mgr(mgr), vertex_shader_path(std::move(vertex_shader_path)),
      fragment_shader_path(std::move(fragment_shader_path)) {}

Program::Builder::Builder(
    AAssetManager *mgr, std::string vertex_shader_path,
    ...
    : mgr(mgr), vertex_shader_path(std::move(vertex_shader_path)),
      fragment_shader_path(std::move(fragment_shader_path)),
      uniforms(std::move(uniforms)), attributes(std::move(attributes)),
      buffers(std::move(buffers)), cube_textures(std::move(cube_textures)),
...
Program::Builder
Program::Builder::add_cube_texture(const std::string &name,
...
std::unique_ptr<Program> Program::Builder::build() {
  ...
  program->vertex_shader_id =
      load_shader(mgr, GL_VERTEX_SHADER, vertex_shader_path);
  program->fragment_shader_id =
      load_shader(mgr, GL_FRAGMENT_SHADER, fragment_shader_path);
  ...
  for (const auto &[name, data] : buffers) {
    ...
    glBufferData(GL_ARRAY_BUFFER, int(data.size()) * BYTES_PER_FLOAT, &data[0],
                 GL_STATIC_DRAW);
  ...
  // cube textures
  std::map<std::string, GLenum> file_to_tex_id =
      Program::get_file_to_texture_id();
  for (const auto &[name, files_path] : cube_textures) {
    ...
    for (const auto &file : files_path) {
      ...
      glTexImage2D(file_to_tex_id[file_name], 0, GL_RGBA, img.width, img.height,
...
void Program::attrib(const std::string &name, const std::string &buffer_name,
  ...
  glVertexAttribPointer(attribute_handles[name], data_size, GL_FLOAT, GL_FALSE,
                        stride, (char *)nullptr + offset);
