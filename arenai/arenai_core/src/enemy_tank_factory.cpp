...
EnemyTankFactory::EnemyTankFactory(
    ...
    : TankFactory(file_reader, tank_prefix_name, chassis_pos, wanted_frame_frequency),
      ...
      curr_frame_upside_down(0), is_dead_already_triggered(false), distance_scale(75.f),
      angle_scale(static_cast<float>(M_PI) / 6.f), has_touch(false),
...
float EnemyTankFactory::get_phi(
    ...
    for (const auto &other: tank_factories) {
        ...
        const float quality_score = 0.5f * angle_quality(angle) * distance_quality(distance)
                                    + 0.3f * angle_quality(angle)
                                    + 0.2f * distance_quality(distance);
