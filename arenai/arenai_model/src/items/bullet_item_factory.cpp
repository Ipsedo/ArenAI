...
std::shared_ptr<Item> BulletItemFactory::make_sphere_item(
    ...
    auto item = std::make_shared<SphereItem>(std::move(name), file_reader, position, scale, mass);
...
std::shared_ptr<Item> BulletItemFactory::make_tetra_item(
    std::string name, const std::shared_ptr<AbstractFileReader> &file_reader, glm::vec3 position,
    glm::vec3 scale, float mass) {
    auto item = std::make_shared<TetraItem>(std::move(name), file_reader, position, scale, mass);
    engine.add_bullet_item(item);
    return item;
}
...
std::shared_ptr<Item> BulletItemFactory::make_cylinder_item(
    std::string name, const std::shared_ptr<AbstractFileReader> &file_reader, glm::vec3 position,
    glm::vec3 scale, float mass) {
    auto item = std::make_shared<CylinderItem>(std::move(name), file_reader, position, scale, mass);
    engine.add_bullet_item(item);
    return item;
}
...
std::shared_ptr<Item> BulletItemFactory::make_height_map_item(
    ...
    auto item =
        std::make_shared<HeightMapItem>(std::move(name), file_reader, height_map_file, pos, scale);
    engine.add_bullet_item(item);
    return item;
}
