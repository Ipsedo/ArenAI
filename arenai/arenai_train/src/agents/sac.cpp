...
agent_response SacAgent::act(const torch::Tensor &vision, const torch::Tensor &sensors) {
    ...
    const auto continuous_action = gaussian_tanh_sample(mu, sigma);
...
void SacAgent::train(
    ...
    for (int e = 0; e < epochs; e++) {
        ...
        torch::Tensor target_q_values;
        {
            ...
            const auto next_continuous_action = gaussian_tanh_sample(next_mu, next_sigma);
            const auto next_continuous_entropy =
                -gaussian_tanh_log_pdf(next_continuous_action, next_mu, next_sigma).sum(-1, true);
        ...
        const auto curr_continuous_action = gaussian_tanh_sample(curr_mu, curr_sigma);
        const auto curr_continuous_entropy =
            -gaussian_tanh_log_pdf(curr_continuous_action, curr_mu, curr_sigma).sum(-1, true);
