...
void NormalizedPotentialRewardReplayBuffer::on_add_step(
...
NormalizedPotentialRewardReplayBuffer::to_output_step(const TorchInputStep &batch_steps) {
    const auto mean = static_cast<float>(running_sum_ / size());
    const auto var = static_cast<float>(running_sum_sq_ / size() - mean * mean);
    const auto std = std::sqrt(std::max(var, 0.f) + 1e-8f);

    const torch::Tensor normalized_potential_reward = (batch_steps.potential_reward - mean) / std;
