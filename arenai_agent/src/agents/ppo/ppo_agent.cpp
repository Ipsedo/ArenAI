...
namespace arenai::agent {
    ...
    TorchAction TorchPpoAgent::act(const TorchState &state, const bool sample) {
...

        {
            ...
            const auto &[mu, kappa, discrete_proba] = actor->act(vision, sensors);

            if (sample) {
                action.continuous_action = beta_law_sample(mu, kappa);
                action.discrete_action = multinomial_sample(discrete_proba);
            } else {
                action.continuous_action = beta_law_mean_action(mu, kappa);
                action.discrete_action = multinomial_max_action(discrete_proba);
            }

            // old log-probabilities, kept for the PPO importance ratio
            continuous_log_prob =
                beta_law_log_proba(action.continuous_action, mu, kappa).sum(-1, true);
