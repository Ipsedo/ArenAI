...
namespace arenai::agent {
    ...
    std::vector<CliField<PpoHyperParams>> ppo_cli_fields() {
        return {
            ...
            {.name = "--target_entropy_init", .member = &PpoHyperParams::target_entropy_init},
            {.name = "--target_entropy_final", .member = &PpoHyperParams::target_entropy_final},
            {.name = "--target_entropy_warmup_steps",
             .member = &PpoHyperParams::target_entropy_warmup_steps},
            {.name = "--target_fire_proba_init", .member = &PpoHyperParams::target_fire_proba_init},
            {.name = "--target_fire_proba_final",
             .member = &PpoHyperParams::target_fire_proba_final},
            {.name = "--target_fire_proba_warmup_steps",
             .member = &PpoHyperParams::target_fire_proba_warmup_steps},
