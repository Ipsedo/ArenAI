        ...
        for (int e = 0; e < epochs; e++) {
            ...
            torch::Tensor target_q_values;
            {
                ...
                const auto soft_value = [&](const TorchState &s) {
                    const auto [mode, concentration, discrete_proba] =
                        actor->act(s.vision, s.proprioception);

                    const auto continuous_action = beta_law_sample(mode, concentration);
            ...
            // policy
            const auto [curr_mode, curr_concentration, curr_discrete_proba] =
                actor->act(state.vision, state.proprioception);

            const auto curr_continuous_action = beta_law_sample(curr_mode, curr_concentration);
            const auto curr_continuous_entropy = beta_law_entropy(curr_mode, curr_concentration);
            ...
            const auto alpha_continuous_loss =
                torch::sum(
                    alpha_continuous->log_alpha()
                        * torch::detach(
                            curr_continuous_entropy - continuous_target_entropy->target_entropy()),
                    -1)
                    .mean();
            ...
            const auto alpha_discrete_loss =
                torch::sum(
                    alpha_discrete->log_alpha()
                        * torch::detach(
                            curr_discrete_entropy - discrete_target_entropy->target_entropy()),
                    -1)
                    .mean();
            ...
            continuous_target_entropy_metric->add(
                continuous_target_entropy->target_entropy().mean().item<float>());
            discrete_target_entropy_metric->add(
                discrete_target_entropy->target_entropy().mean().item<float>());
