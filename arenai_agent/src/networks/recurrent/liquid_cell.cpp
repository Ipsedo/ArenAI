...
LiquidCell::LiquidCell(
    ...
    : a(register_parameter("a", torch::ones({1, neuron_number}))),
      ...
      unfolding_steps(unfolding_steps), delta_t(delta_t), neuron_number(neuron_number),
...
torch::Tensor LiquidCell::forward(const torch::Tensor &inputs) {
    TORCH_CHECK(
        inputs.sizes().size() == 3,
...
LiquidCell::forward_step(const torch::Tensor &x_t, const torch::Tensor &input_t) {
    const auto x_t_next = next_x(x_t, input_t);
    return {to_output->forward(x_t_next), x_t_next};
}

torch::Tensor LiquidCell::next_x(const torch::Tensor &x_t, const torch::Tensor &input_t) const {
    auto x_t_next = x_t;
...
torch::Tensor LiquidCell::get_first_x(int batch_size) {
