...
#include <arenai_agent_tests/tests_e2e/interceptor_agent.h>
...
// ========================================================================
// End-to-end: TrainTankEnvironment -> AbstractAgent::act()
// The interceptor plays the network's role; whatever reaches act() is what
// a real agent would have been fed by the environment loop.
// ========================================================================
...
namespace {
    ...
    std::unique_ptr<TrainTankEnvironment> make_environment() {
        auto env = std::make_unique<TrainTankEnvironment>(
            view::make_vulkan_backend(), NB_TANKS, ARENAI_ASSETS_DIR, FREQUENCY, 1000,
            VISION_HEIGHT, VISION_WIDTH, 1);
        env->seed(42);
        return env;
    }

    // the run_game()/train loop shape: act on the current states, step the
    ...
    std::filesystem::path repository_root() {
        // arenai_agent/tests/src/tests_e2e/ -> repository root
...
TEST(AgentInputEndToEnd, AgentReceivesValidStates) {
...
TEST(AgentInputEndToEnd, AgentInputMatchesGoldenImages) {
    ...
    // at reset the scene is the one arenai_core recorded: what reaches the
    // agent must match those references (owned and regenerated by arenai_core)
#ifndef ARENAI_REGENERATE_GOLDEN_IMAGES
    for (int tank_idx = 0; tank_idx < NB_TANKS; tank_idx++) {
        const auto golden_path = root / "arenai_core" / "tests" / "resources" / "golden_images"
                                 / ("golden_env_reset_tank_" + std::to_string(tank_idx) + ".json");

        ASSERT_TRUE(std::filesystem::exists(golden_path)) << golden_path;

        expect_matches_golden(
            golden_path, agent.received_states.front()[tank_idx].vision.pixels,
            "reset tank " + std::to_string(tank_idx));
    }
#endif

    // after NB_STEPS neutral steps: references owned by arenai_agent
    for (int tank_idx = 0; tank_idx < NB_TANKS; tank_idx++) {
        const auto golden_path = root / "arenai_agent" / "tests" / "resources" / "golden_images"
                                 / ("golden_agent_input_step" + std::to_string(NB_STEPS) + "_tank_"
                                    + std::to_string(tank_idx) + ".json");

        const auto &pixels = agent.received_states.back()[tank_idx].vision.pixels;
        ...
        if (std::filesystem::exists(golden_path)) {
            ...
            expect_matches_golden(
                golden_path, pixels,
                "step " + std::to_string(NB_STEPS) + " tank " + std::to_string(tank_idx));
        } else {
