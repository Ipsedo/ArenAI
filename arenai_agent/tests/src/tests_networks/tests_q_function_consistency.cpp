...
TEST_F(ActorGradientTest, GradientFlowsThroughActor) {
    ...
    const auto [mode, concentration, discrete] = actor.act(vision, sensors);
    const auto loss = mode.sum() + concentration.sum() + discrete.sum();
...
TEST_F(ActorGradientTest, ActorWeightsChangeAfterOptimStep) {
    ...
    const auto [mode, concentration, discrete] = actor.act(vision, sensors);
    const auto loss = -(mode.sum() + concentration.log().sum());
