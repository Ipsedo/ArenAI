...
namespace arenai::view {
    ...
    // the public API (include/) stays untouched. The shadow-map sampler and
    // the per-draw shadow matrix travel through the renderer's set 0 (written
    // by the renderer before each call), not through parameters.
