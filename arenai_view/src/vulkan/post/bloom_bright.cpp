...
namespace arenai::view {
    ...
    BloomBrightEffect::BloomBrightEffect(
        ...
        : VulkanPostEffect(
            device, descriptors, "bloom_bright_fs.glsl", 1, 0,
            {{.format = VK_FORMAT_R8G8B8A8_UNORM, .size_divisor = 2}}, width, height) {}

    void BloomBrightEffect::render(FrameContext &context) {
        run_pass(context, 0, {context.scene}, nullptr);

        context.textures[PostTexture::bloom_bright] = target(0);
    }
