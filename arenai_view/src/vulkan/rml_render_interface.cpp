...
namespace arenai::view {
    ...
    void RmlVulkanRenderInterface::drain_retired(const bool everything) {
        std::erase_if(retired_, [&](const Retired &retired) {
            return everything || frame_counter_ - retired.frame >= WindowFrameContext::FRAME_SLOTS;
        });
    }
    ...
    void RmlVulkanRenderInterface::ReleaseGeometry(const Rml::CompiledGeometryHandle geometry) {
        ...
        retired_.push_back({frame_counter_, std::unique_ptr<CompiledGeometry>(compiled), nullptr});
    }
    ...
    void RmlVulkanRenderInterface::ReleaseTexture(const Rml::TextureHandle texture) {
        ...
        retired_.push_back({frame_counter_, nullptr, std::unique_ptr<RmlTexture>(rml_texture)});
    }
