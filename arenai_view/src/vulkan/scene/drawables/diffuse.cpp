...
namespace arenai::view {
    ...
    void VulkanDiffuse::ensure_common() {
        ...
        vertices_ = std::make_unique<VulkanBuffer>(
            context_->device(), context_->upload_pool(), vbo_data_.data(),
            vbo_data_.size() * sizeof(float), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);

        material_ = std::make_unique<HostVisibleBuffer>(
            context_->device(), sizeof(glm::vec4), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
        ...
        material_layout_ =
            DescriptorLayoutBuilder()
                ...
                .build(context_->device()->handle());
        material_set_ = context_->descriptors().allocate(material_layout_);
        write_buffer_descriptor(
            context_->device()->handle(), material_set_, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
    ...
    void VulkanDiffuse::ensure_plain_pipeline() {
        ...
        plain_pipeline_ = PipelineBuilder()
                              ...
                              .build(context_->device(), plain_pipeline_layout_);
    }
    ...
    void VulkanDiffuse::ensure_shadow_pipeline() {
        ...
        shadow_pipeline_ = PipelineBuilder()
                               ...
                               .depth_format(context_->scene_depth_format())
                               .samples(context_->scene_samples())
                               .build(context_->device(), shadow_pipeline_layout_);
    }
    ...
    void VulkanDiffuse::draw(
        ...
        record_draw(
            plain_pipeline_, plain_pipeline_layout_, context_->scene_frame().set0_plain, 0,
    ...
    void VulkanDiffuse::draw_with_shadow(
        ...
        record_draw(
            shadow_pipeline_, shadow_pipeline_layout_, context_->scene_frame().set0_shadow, 1,
            mvp_matrix, mv_matrix);
    }
