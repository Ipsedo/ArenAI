...
#include <arenai_view_tests/test_offscreen.h>

using namespace arenai;
using namespace arenai::view;

TEST_P(OffscreenClearColorParam, ClearColorNoDrawable) {
    const auto [width, height] = GetParam();

    const auto backend = view::make_vulkan_backend();
...
TEST_P(OffscreenClearColorParam, PixelLayoutCHW) {
    const auto [width, height] = GetParam();
...
INSTANTIATE_TEST_SUITE_P(
    TestOffscreenClearColor, OffscreenClearColorParam,
    testing::Values(
