...
int main(int argc, char **argv) {
    ...
    // model
    parser.add_argument("--hidden_size_latent").scan<'i', int>().default_value(160);
    parser.add_argument("--hidden_size").scan<'i', int>().default_value(300);
    parser.add_argument("--tau").scan<'g', float>().default_value(1e-3f);
    parser.add_argument("--gamma").scan<'g', float>().default_value(0.95f);
    ...
    train_main(
        {parser.get<int>("--hidden_size_latent"), parser.get<int>("--hidden_size"),
